...
*** TIMEFRAME CHÍNH: 1D (Daily) — 4H chỉ xác nhận entry ***
Kết hợp: Elliott Wave Structure + Wyckoff 1D + RSI/MACD Divergence 1D
         + Market Structure Break 1D + Volume Climax 1D + Fibonacci 4H
"""
...
def wave_score(df1h: pd.DataFrame, df4h: pd.DataFrame,
    ...
    Max score = 15
    *** 1D LÀM CHÍNH — 4H xác nhận entry — 1H lọc candle/oscillator ***

    Phân bổ điểm:
      Nhóm 1 — Xu hướng 1D (EMA + MSB + Wyckoff 1D)   : tối đa 6đ  ← CHÍNH
      Nhóm 2 — Divergence 1D (RSI/MACD/OBV trên 1D)   : tối đa 4đ  ← CHÍNH
      Nhóm 3 — Xác nhận 4H (EMA + MSB + Fib)          : tối đa 2đ  ← PHỤ
      Nhóm 4 — Volume Climax 1D + Oscillator 1D        : tối đa 2đ  ← CHÍNH
      Nhóm 5 — Candle 1D + S/R 4H + Volume surge 1D   : tối đa 1đ  ← HỖ TRỢ
    """
    r    = df1h.iloc[-1]
    r4   = df4h.iloc[-1]
    rd   = df1d.iloc[-1]
    ...
    # ══════════════════════════════════════════════════════════
    # NHÓM 1: XU HƯỚNG LỚN TRÊN 1D — nền tảng chính (tối đa 6đ)
    # ══════════════════════════════════════════════════════════

    # 1a. EMA 50/200 ngày — xu hướng dài hạn (3đ)
    ...
    # 1b. EMA 9/21 ngày — momentum trung hạn (1đ)
    bull1d_fast = rd["ema9"] > rd["ema21"]
    detail["EMA 9/21 (1D)"] = "✅ Bull momentum" if bull1d_fast else "❌ Bear momentum"
    buy_sc  += 1 if bull1d_fast     else 0
    sell_sc += 1 if not bull1d_fast else 0
    if bull1d_fast:  patterns.append("📈 EMA 9>21 Bullish 1D")
    if not bull1d_fast: patterns.append("📉 EMA 9<21 Bearish 1D")

    # 1c. Market Structure Break trên 1D (2đ)
    msb1d = detect_market_structure_break(df1d)
    if msb1d["choch_bull"]:
        detail["Market Structure (1D)"] = "✅ CHoCH Bullish 1D — Đảo chiều tăng lớn"
        buy_sc += 2
        patterns.append("🔄 CHoCH Bullish 1D")
    elif msb1d["bull_msb"]:
        detail["Market Structure (1D)"] = "✅ HH+HL Bullish 1D"
        buy_sc += 1
        patterns.append("🏗️ Bullish Structure 1D")
    if msb1d["choch_bear"]:
        detail["Market Structure (1D)"] = "❌ CHoCH Bearish 1D — Đảo chiều giảm lớn"
        sell_sc += 2
        patterns.append("🔄 CHoCH Bearish 1D")
    elif msb1d["bear_msb"]:
        detail["Market Structure (1D)"] = "❌ LL+LH Bearish 1D"
        sell_sc += 1
        patterns.append("🏗️ Bearish Structure 1D")

    # 1d. Wyckoff trên 1D (2đ)
    wyckoff1d = detect_wyckoff(df1d)
    if wyckoff1d["accumulation"]:
        detail["Wyckoff (1D)"] = "✅ Accumulation Zone 1D"
        buy_sc += 1
        patterns.append("🏦 Wyckoff Accumulation 1D")
    if wyckoff1d["spring"]:
        detail["Wyckoff Spring (1D)"] = "✅ Spring 1D — Đáy sóng lớn"
        buy_sc += 2
        patterns.append("🌱 Spring 1D")
    if wyckoff1d["distribution"]:
        detail["Wyckoff (1D)"] = "❌ Distribution Zone 1D"
        sell_sc += 1
        patterns.append("🏦 Wyckoff Distribution 1D")
    if wyckoff1d["utad"]:
        detail["Wyckoff UTAD (1D)"] = "❌ UTAD 1D — Đỉnh sóng lớn"
        sell_sc += 2
        patterns.append("⚠️ UTAD 1D")

    # ══════════════════════════════════════════════════════════
    # NHÓM 2: DIVERGENCE TRÊN 1D — tín hiệu đảo chiều ngày (tối đa 4đ)
    # ══════════════════════════════════════════════════════════
    rsi_div1d  = detect_rsi_divergence(df1d, lookback=30)
    macd_div1d = detect_macd_divergence(df1d, lookback=30)
    obv_div1d  = detect_obv_divergence(df1d, lookback=30)

    div_bull1d = rsi_div1d["bull_div"] or rsi_div1d["bull_hidden"]
    div_bear1d = rsi_div1d["bear_div"] or rsi_div1d["bear_hidden"]

    if div_bull1d:
        detail["RSI Divergence (1D)"] = "✅ Bullish 1D — Đáy sóng ngày xác nhận"
        buy_sc += 2
        patterns.append("📊 RSI Bull Divergence 1D")
    if div_bear1d:
        detail["RSI Divergence (1D)"] = "❌ Bearish 1D — Đỉnh sóng ngày xác nhận"
        sell_sc += 2
        patterns.append("📊 RSI Bear Divergence 1D")

    if macd_div1d["bull_div"]:
        detail["MACD Divergence (1D)"] = "✅ Bullish 1D"
        buy_sc += 1
        patterns.append("📉 MACD Bull Divergence 1D")
    if macd_div1d["bear_div"]:
        detail["MACD Divergence (1D)"] = "❌ Bearish 1D"
        sell_sc += 1
        patterns.append("📉 MACD Bear Divergence 1D")

    if obv_div1d["bull"]:
        detail["OBV (1D — Smart Money)"] = "✅ Smart money tích lũy theo ngày"
        buy_sc += 1
        patterns.append("📦 OBV Accumulation 1D")
    if obv_div1d["bear"]:
        detail["OBV (1D — Smart Money)"] = "❌ Smart money phân phối theo ngày"
        sell_sc += 1
        patterns.append("📦 OBV Distribution 1D")

    # RSI 1D Oversold / Overbought (1đ)
    rsi_1d  = rd["rsi"]
    mfi_1d  = rd["mfi"]
    stk_1d  = rd["stoch_k"]
    wpr_1d  = rd["wpr"]
    oversold_1d   = (rsi_1d < 35 and stk_1d < 20)
    overbought_1d = (rsi_1d > 65 and stk_1d > 80)
    detail["Oscillators (1D)"] = (
        f"RSI={rsi_1d:.0f} StochK={stk_1d:.0f} WPR={wpr_1d:.0f} MFI={mfi_1d:.0f} "
        f"{'✅ Oversold 1D' if oversold_1d else ('❌ Overbought 1D' if overbought_1d else '⚠️ Normal')}"
    )
    if oversold_1d:
        buy_sc += 1
        patterns.append("📉 Oversold 1D")
    if overbought_1d:
        sell_sc += 1
        patterns.append("📈 Overbought 1D")

    # ══════════════════════════════════════════════════════════
    # NHÓM 3: XÁC NHẬN 4H — chỉ dùng để lọc entry (tối đa 2đ)
    # ══════════════════════════════════════════════════════════
    bull4h = r4["ema50"] > r4["ema200"]
    align_bull = bull1d and bull4h          # 1D và 4H cùng hướng tăng
    align_bear = (not bull1d) and (not bull4h)
    detail["EMA 50/200 (4H — xác nhận)"] = (
        f"{'✅ Bull' if bull4h else '❌ Bear'} "
        f"{'— Đồng thuận 1D ✅' if (align_bull or align_bear) else '— Lệch chiều 1D ⚠️'}"
    )
    buy_sc  += 1 if align_bull else 0
    sell_sc += 1 if align_bear else 0
    if align_bull: patterns.append("🔗 4H đồng thuận Bull 1D")
    if align_bear: patterns.append("🔗 4H đồng thuận Bear 1D")
    ...
    if fib["fib_support"]:
        ...
        buy_sc += 1
        patterns.append(f"🌀 Fib {fib['fib_level']:.3f} Support 4H")
    if fib["fib_resistance"]:
        detail["Fibonacci (4H)"] = f"❌ Fib {fib['fib_level']:.3f} Resistance"
        sell_sc += 1
        patterns.append(f"🌀 Fib {fib['fib_level']:.3f} Resistance 4H")

    # ══════════════════════════════════════════════════════════
    # NHÓM 4: VOLUME CLIMAX 1D (tối đa 2đ)
    # ══════════════════════════════════════════════════════════
    climax1d = detect_volume_climax(df1d)
    if climax1d["sell_climax"]:
        detail["Volume Climax (1D)"] = "✅ Selling Climax 1D — Panic bán xong"
        buy_sc += 2
        patterns.append("💥 Selling Climax 1D")
    if climax1d["buy_climax"]:
        detail["Volume Climax (1D)"] = "❌ Buying Climax 1D — FOMO xong"
        sell_sc += 2
        patterns.append("💥 Buying Climax 1D")

    # ══════════════════════════════════════════════════════════
    # NHÓM 5: HỖ TRỢ — Candle 1D + S/R 4H + Volume surge 1D (tối đa 1đ)
    # ══════════════════════════════════════════════════════════
    candle1d = detect_candle_reversal(df1d)
    if candle1d["bull_reversal"]:
        detail["Candle Pattern (1D)"] = "✅ " + " + ".join(candle1d["patterns"])
        buy_sc += 1
        patterns.extend([p + " (1D)" for p in candle1d["patterns"]])
    if candle1d["bear_reversal"]:
        detail["Candle Pattern (1D)"] = "❌ " + " + ".join(candle1d["patterns"])
        sell_sc += 1
        patterns.extend([p + " (1D)" for p in candle1d["patterns"]])

    sr4h = detect_support_resistance_test(df4h)
    if sr4h["bounce_up"]:
        detail["S/R Test (4H)"] = "✅ Bounce từ Support 4H"
        buy_sc += 1
        patterns.append("🎯 Bounce Support 4H")
    if sr4h["bounce_down"]:
        detail["S/R Test (4H)"] = "❌ Reject từ Resistance 4H"
        sell_sc += 1
        patterns.append("🎯 Reject Resistance 4H")

    vol_surge_1d = rd["vol_ratio"] > 2.0
    detail["Volume Surge (1D)"] = f"x{rd['vol_ratio']:.1f} avg {'✅' if vol_surge_1d else '⚠️'}"
    buy_sc  += 1 if vol_surge_1d else 0
    sell_sc += 1 if vol_surge_1d else 0
...
def wave_levels(df4h: pd.DataFrame, df1d: pd.DataFrame, side: str) -> tuple:
    """
    TP/SL tính theo ATR 1D (làm chính) — phù hợp tín hiệu ngày.
    Entry zone dùng ATR 4H để tinh chỉnh vào lệnh.
    """
    price = df1d["close"].iloc[-1]   # Dùng giá đóng cửa nến 1D
    atr1d = df1d["atr"].iloc[-1]     # ATR 1D — đơn vị đo sóng ngày
    atr4h = df4h["atr"].iloc[-1]     # ATR 4H — chỉ dùng cho entry zone

    if side == "BUY":
        elo  = price - 0.3 * atr4h          # Entry thấp (ATR 4H ± nhỏ)
        ehi  = price + 0.3 * atr4h          # Entry cao
        sl   = price - 1.0 * atr1d          # SL theo sóng ngày
        tp1  = price + 1.0 * atr1d          # TP1 = 1:1 (chốt 30%)
        tp2  = price + 2.0 * atr1d          # TP2 = 1:2 (chốt 40%)
        tp3  = price + 3.5 * atr1d          # TP3 = 1:3.5 (để chạy sóng)
    else:
        elo  = price - 0.3 * atr4h
        ehi  = price + 0.3 * atr4h
        sl   = price + 1.0 * atr1d
...
async def analyze(exchange, sem, symbol: str, vol24h: float) -> Optional[WaveSignal]:
    async with sem:
        try:
            ...
            price = df1d["close"].iloc[-1]   # Giá đóng nến ngày — làm chính

            # Xác định wave_type dựa trên MSB 1D
            msb1d = detect_market_structure_break(df1d)

            if bs >= WAVE_BUY_THRESHOLD:
                lv = wave_levels(df4h, df1d, "BUY")
                elo,ehi,sl,tp1,tp2,tp3,slp,t1p,t2p,t3p,rr = lv
                wt = "WAVE_BOTTOM" if bs >= 10 else ("MSB_BULL" if msb1d["bull_msb"] else "WAVE_BOTTOM")
            ...
            if ss >= WAVE_SELL_THRESHOLD:
                ...
                elo,ehi,sl,tp1,tp2,tp3,slp,t1p,t2p,t3p,rr = lv
                wt = "WAVE_TOP" if ss >= 10 else ("MSB_BEAR" if msb1d["bear_msb"] else "WAVE_TOP")
...
async def wave_scan(bot: Bot):
    ...
    summary  = (
        ...
        f"📊 <i>Score ≥ 8/15 mới gửi | TF chính: 1D | Kết hợp: Divergence 1D + Wyckoff 1D + MSB 1D + Fibonacci 4H + Volume Climax 1D</i>"
    )
...
# ─── COMMAND HANDLERS ──────────────────────────────────────────────────────────
# Ngăn chạy nhiều scan cùng lúc khi user spam /start
_scan_lock = asyncio.Lock()

async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Quét ngay khi nhận /start"""
    if _scan_lock.locked():
        await update.message.reply_text(
            "⏳ Đang quét... vui lòng chờ lần quét hiện tại hoàn thành.",
            parse_mode=ParseMode.HTML
        )
        return

    await update.message.reply_text(
        "🌊 <b>Đang quét toàn thị trường...</b>\n"
        "⏱ Thường mất 30–90 giây, vui lòng chờ.",
        parse_mode=ParseMode.HTML
    )
    async with _scan_lock:
        await wave_scan(context.bot)


async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Hiển thị hướng dẫn sử dụng"""
    help_text = (
        "🌊 <b>WAVE SIGNAL BOT — Hướng dẫn</b>\n\n"
        "📌 <b>Lệnh có sẵn:</b>\n"
        "  /start — Quét toàn thị trường ngay lập tức\n"
        "  /help  — Hiển thị hướng dẫn này\n\n"
        "🕒 <b>Tự động quét:</b> Mỗi 4 giờ (0h, 4h, 8h, 12h, 16h, 20h UTC)\n"
        "⚡ <b>Timeframe chính:</b> 1D (Daily) — 4H chỉ xác nhận entry\n\n"
        "📊 <b>Điều kiện gửi tín hiệu:</b>\n"
        "  • Score ≥ 8/15\n"
        "  • Volume 24h ≥ 5M USDT\n"
        "  • Kết hợp: Divergence + Wyckoff + MSB + Fibonacci + Volume Climax\n\n"
        "⚠️ <i>Chỉ mang tính tham khảo. Không phải lời khuyên đầu tư.</i>"
    )
...
async def scheduled_scan(context: ContextTypes.DEFAULT_TYPE):
    """Wrapper cho APScheduler gọi wave_scan với bot từ Application"""
    if _scan_lock.locked():
        log.info("Bỏ qua lịch scan vì đang có scan chạy.")
        return
    async with _scan_lock:
        await wave_scan(context.bot)
...
def main():
    ...
    # Lên lịch quét tự động mỗi 4H qua job queue của Application
    job_queue = app.job_queue
    # Quét ngay sau 5 giây khi khởi động
    job_queue.run_once(scheduled_scan, when=5)
    # Quét định kỳ mỗi 4 tiếng
    job_queue.run_repeating(scheduled_scan, interval=4*3600, first=5)

    log.info("🌊 Wave Signal Bot started — /start để quét ngay, tự động mỗi 4 giờ")
