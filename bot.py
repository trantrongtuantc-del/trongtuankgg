...
import config as cfg  # load dotenv here first

# ── Validate token EARLY with clear message ───────────────
if not cfg.TELEGRAM_TOKEN:
    print("=" * 60)
    print("LỖI: TELEGRAM_TOKEN chưa được đặt!")
    print("Cách sửa:")
    print("  1. Railway → Service → Variables → thêm TELEGRAM_TOKEN")
    print("  2. Hoặc tạo file .env với: TELEGRAM_TOKEN=your_token")
    print("=" * 60)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Bot quét tín hiệu mua/bán tự động cho top 500 crypto.\n\n"
        "📋 Lệnh chính:\n"
        "/scan    — Quét ngay\n"
...
async def cmd_scan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async def progress(done, total):
        if done % 100 == 0:
            try:
                await msg.edit_text(
                    f"🔍 Đang quét <b>{done}/{total}</b> coins...",
                    parse_mode=ParseMode.HTML,
                )
            except Exception:
                pass
    ...
    try:
        ...
        await msg.edit_text(f"❌ Lỗi scan: {e}")
    finally:
...
async def _send_top(update: Update, direction: Optional[str], n: int):
    if not state.last_result:
        await update.message.reply_text("⚠️ Chưa có dữ liệu. Dùng /scan trước.")
        return
    ...
    await update.message.reply_text(
        f"📊 <b>Top {min(n,len(sigs))} tín hiệu{' '+direction if direction else ''}</b>\n"
        f"⏰ Quét: {_now_str()} ({age}s trước)",
        parse_mode=ParseMode.HTML,
    )
    for s in sigs[:n]:
        await update.message.reply_text(s.to_message(), parse_mode=ParseMode.HTML)
...
async def cmd_config(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"RSI        : {cfg.RSI_PERIOD} OB:{cfg.RSI_OB} OS:{cfg.RSI_OS}\n"
...
async def cmd_setmin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        val = int(ctx.args[0])
        assert 1 <= val <= 10
        state.min_score = state.min_master = val
        await update.message.reply_text(f"✅ Score tối thiểu = <b>{val}</b>", parse_mode=ParseMode.HTML)
    except Exception:
        await update.message.reply_text("⚠️ Cú pháp: /setmin <số 1-10>")
...
async def cmd_pause(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    state.paused = True
    await update.message.reply_text("⏸ Auto-scan <b>tạm dừng</b>.", parse_mode=ParseMode.HTML)

async def cmd_resume(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not _is_admin(update.effective_chat.id):
        await update.message.reply_text("⛔ Không có quyền."); return
    state.paused = False
    await update.message.reply_text("▶️ Auto-scan <b>tiếp tục</b>.", parse_mode=ParseMode.HTML)

# ── /unsub ────────────────────────────────────────────────
async def cmd_unsub(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    state.subscribed.discard(update.effective_chat.id)
    await update.message.reply_text("🔕 Đã hủy đăng ký nhận tín hiệu.")
...
async def cmd_symbols(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        ticks = await exch.fetch_tickers()
        await exch.close()
        pairs = [
            (s, t.get("quoteVolume") or 0)
            for s, t in ticks.items()
            if s.endswith(f"/{cfg.QUOTE_ASSET}") and (t.get("quoteVolume") or 0) >= cfg.MIN_VOLUME_USDT
        ]
        pairs.sort(key=lambda x: x[1], reverse=True)
        lines = "\n".join(f"{i+1}. {s} (${v:,.0f})" for i,(s,v) in enumerate(pairs[:20]))
        await msg.edit_text(
            f"🪙 <b>Top 20 / {len(pairs)} symbols đủ điều kiện</b>\n\n"
            f"<code>{lines}</code>\n\nSẽ quét: <b>{min(len(pairs), cfg.TOP_N_COINS)}</b> coins",
            parse_mode=ParseMode.HTML,
        )
    except Exception as e:
...
# ── Main ──────────────────────────────────────────────────
def main():
    logger.info(f"Token loaded: {cfg.TELEGRAM_TOKEN[:10]}...")
    logger.info(f"TF:{cfg.TIMEFRAME} | Top:{cfg.TOP_N_COINS} | Score≥{cfg.MIN_MASTER_SCORE}")
