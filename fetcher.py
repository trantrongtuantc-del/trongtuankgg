...
def get_top_symbols(limit: int = 500) -> List[str]:
    """
    Lấy danh sách top symbols USDT theo volume 24h từ Binance.
    """
    try:
        ...
        # Fallback: danh sách cứng top coins
        return [
...
def batch_fetch(
    ...
    interval: str = "1h",
    limit: int = 250,
    delay: float = 0.12   # ~8 req/s — dưới rate limit Binance (1200/min)
) -> dict:
    """
    Lấy nến cho nhiều symbol cùng lúc (tuần tự có delay).
